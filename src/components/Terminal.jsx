...
import "./Terminal.css";
import { commandsPromise, getCommandList, argumentTypes, getCommandHandler } from "../commands/index.js";
import { ANSI_COLORS, ArgumentError, getDate, InterruptError } from "../utils.js";
...
class Terminal {
    ...
    promptNumber(promptMessage = "Enter a number: ", signal) {
...
function TerminalComponent() {
    ...
    const [commandMetadata, setCommandMetadata] = createSignal(null);
    ...
    const parseArguments = async (args, argumentDefinitions) => {
        const parsedArgs = {};
        const argsCopy = [...args];
        ...
        for (const argDef of argumentDefinitions) {
            ...
            let flagIndex = argsCopy.findIndex(a => a === longFlag || (shortFlag && a === shortFlag));
            ...
            if (flagIndex !== -1) {
                ...
                const typeDef = typeof argDef.type === 'string' ? argumentTypes[argDef.type] : argDef.type;
                const typeName = typeof argDef.type === 'string' ? argDef.type : (argDef.type.typeName || 'custom');
                if (!typeDef || !(await typeDef.validate(value))) {
                    return { error: `Invalid value for '${argDef.name}': expected ${typeName}, got '${value}'.` };
                }
    ...
    createEffect(() => {
        const updateSuggestionsAndFeedback = async () => {
            ...
            if (prevPart?.startsWith('-') && command.arguments.some(a => (a.name === prevPart.replace(/^-+/, '') || a.shorthand === prevPart.replace(/^-+/, '')) && a.type !== 'boolean')) {
                ...
                parts.slice(1, -1).forEach((part, i) => {
                    if (part.startsWith('-')) {
