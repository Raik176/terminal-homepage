...
export class Terminal {
    ...
    public prompt(promptMessage: string, signal?: AbortSignal): Promise<string | null> {
    ...
    public promptNumber(promptMessage: string = "Enter a number: ", signal?: AbortSignal): Promise<number> {
...
const TerminalComponent: Component = () => {
    ...
    const terminal = new Terminal(setHistory, createSignal(shellPrompt), createStore<string[]>([]));
    ...
    const validateAndParseArgument = async (
        ...
        const typeDef = typeof argDef.type === 'string'
            ? argumentTypes[argDef.type as keyof typeof argumentTypes]
            : argDef.type;

        const typeName = typeof argDef.type === 'string'
            ? argDef.type
    ...
    const parseArguments = async (args: string[], argumentDefinitions: readonly any[]): Promise<{ args?: Record<string, any>; error?: string }> => {
        ...
        for (const argDef of argumentDefinitions) {
            ...
            const flagIndex = argsCopy.findIndex(a => a === longFlag || (shortFlag && a === shortFlag));
            ...
            if (flagIndex !== -1) {
                ...
                const errorMessage = await validateAndParseArgument(value, argDef, parsedArgs);
                if (errorMessage) {
        ...
        const positionalArgDefs = argumentDefinitions.filter(def => !parsedArgs.hasOwnProperty(def.name));
        for (let i = 0; i < Math.min(argsCopy.length, positionalArgDefs.length); i++) {
    ...
    const handleCommand = async (cmd: string, addToHistory: boolean = false): Promise<void> => {
        if (cmd.trim() === "") return;
        ...
        if (command) {
            ...
            try {
                ...
                if (command.arguments) {
                    const parsed = await parseArguments(args, command.arguments);
                    if (parsed.error) {
                        terminal.error(new ArgumentError(parsed.error));
                    } else {
                        cmdArgs = parsed.args!;
                        const handler = await getCommandHandler(commandName);
                        await handler(terminal, cmdArgs, controller.signal, allCommands);
                    }
                } else {
                    const handler = await getCommandHandler(commandName);
                ...
                if (err.name === 'AbortError') {
                    terminal.error(new InterruptError("Command interrupted by user."));
                } else {
                    terminal.error(err);
                }
            } finally {
        ...
        switch (e.key) {
            ...
            case "ArrowUp":
                ...
                if (historyIndex() < terminal.commandHistory.length - 1) {
                    ...
                    setInput(terminal.commandHistory[terminal.commandHistory.length - 1 - newIndex]);
                }
                break;
            case "ArrowDown":
                e.preventDefault();
    ...
    createEffect(() => {
        const updateSuggestionsAndFeedback = async () => {
            ...
            if (parts.length === 1) {
                ...
                const matchingCommands = commandList.filter((cmd) => cmd.startsWith(commandName));

                if (matchingCommands.length > 0 && matchingCommands[0] !== commandName) {
            ...
            if (prevPart?.startsWith('-') && command.arguments.some(a => (a.name === prevPart.replace(/^-+/, '') || a.shorthand === prevPart.replace(/^-+/, '')) && a.type !== 'boolean')) {
                const flagName = prevPart.replace(/^-+/, '');
                argDef = command.arguments.find(a => a.name === flagName || a.shorthand === flagName);
            } else {
                const providedFlags = new Set();
                parts.slice(1, -1).forEach((part) => {
                    if (part.startsWith('-')) {
                        const flagName = part.replace(/^-+/, '');
                        const def = command.arguments!.find(a => a.name === flagName || a.shorthand === flagName);
                        if(def) providedFlags.add(def.name);
                    }
                });
                const positionalArgDefs = command.arguments.filter(a => !providedFlags.has(a.name));
            ...
            if (argDef) {
                const typeDef = typeof argDef.type === 'string' ? argumentTypes[argDef.type as keyof typeof argumentTypes] : argDef.type;
                if (typeDef?.suggestions) {
                    const suggestions = await typeDef.suggestions(argInput);
                    if (suggestions.length > 0) {
                        const baseInput = currentInput.substring(0, currentInput.lastIndexOf(argInput));
                        setSuggestion(baseInput + suggestions[0]);
                    } else {
                        setSuggestion("");
                    }
                    const typeName = typeof argDef.type === 'string' ? argDef.type : (argDef.type.typeName || 'custom');
                    const argName = argDef.shorthand ? `-${argDef.shorthand}, --${argDef.name}` : `--${argDef.name}`;
    ...
    onMount(async () => {
        ...
        const startupCommands = localStorage.getItem("start-commands")!.split(",").filter(c => c);
                    ...
                    for (let j = 0; j < codeParts.length; j++) {
                        ...
                        if (num >= 30 && num <= 37) {
                            ...
                            if (type === 5) {
                                ...
                                if (isBg) currentBgColor = `background-color: ${color};`;
                                ...
                                if (isBg) currentBgColor = `background-color: ${rgbColor};`;
    ...
    const getColor = (num: number) => {
        const colors = ["var(--black)", "var(--red)", "var(--green)", "var(--yellow)", "var(--blue)", "var(--magenta)", "var(--cyan)", "var(--white)"];
        return colors[num - 30];
    };

    const getBrightColor = (num: number) => {
        const colors = ["var(--bright-black)", "var(--bright-red)", "var(--bright-green)", "var(--bright-yellow)", "var(--bright-blue)", "var(--bright-magenta)", "var(--bright-cyan)", "var(--bright-white)"];
    ...
    const renderOutput = (output: OutputItem | null): JSX.Element | (JSX.Element | string)[] | undefined => {
        if (output === null) return;
        if (typeof output === 'object' && output.html) {
            createEffect(() => {
                if (terminalElement) {
                    const codeBlocks = terminalElement.querySelectorAll("pre code");
    ...
    return (
        ...
        >
            <div class="flex-grow overflow-auto">
                ...
                >
                    <For each={history}>{(item) => <div>{renderOutput(item)}</div>}</For>
                        ...
                        <>
                            <div class="flex items-center w-full">
                                ...
                                <div class="flex-grow relative">
                                    ...
                                    />
                                    {suggestion() && (
                                        <span class="absolute left-0 top-0 text-gray-500 pointer-events-none" style={`padding-left: ${input().length}ch;`}>
