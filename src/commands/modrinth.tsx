...
const UserProfile: Component<UserProfileProps> = (props) => {
    ...
    return (
        <div class="mb-6 p-4 border rounded-lg flex items-center gap-5" style={{ 'border-color': 'var(--bright-black)' }}>
            <img src={props.user.avatar_url} alt={`${props.user.username}'s avatar`} class="w-24 h-24 rounded-full flex-shrink-0 object-cover"/>
            <div class="flex-grow">
                ...
                {props.user.bio && <p class="text-sm my-2" style={{ 'color': 'var(--text-color)' }}>{props.user.bio}</p>}
                <div class="text-xs mt-3 flex flex-wrap gap-x-4 gap-y-2" style={{ 'color': 'var(--bright-black)' }}>
                    <span><span style={{'color': 'var(--text-color)'}}>📦</span> Projects: <span class="font-semibold" style={{ 'color': 'var(--magenta)' }}>{props.projectCount}</span></span>
                    <span><span style={{'color': 'var(--text-color)'}}>📈</span> Total Downloads: <span class="font-semibold" style={{ 'color': 'var(--yellow)' }}>{props.totalDownloads.toLocaleString()}</span></span>
                    <span><span style={{'color': 'var(--text-color)'}}>📅</span> Created: <span class="font-semibold" style={{ 'color': 'var(--cyan)' }}>{formatDateWithRelativeTime(props.user.created)}</span></span>
                </div>
            </div>
        </div>
    );
...
const ModrinthProject: Component<{ project: ModrinthProjectData }> = (props) => {
    ...
    return (
        <div class="my-3 p-4 border rounded-lg flex items-start gap-4" style={{ 'border-color': 'var(--bright-black)' }}>
            {props.project.icon_url && <img src={props.project.icon_url} alt={`${props.project.title} icon`} class="w-20 h-20 rounded-md flex-shrink-0 object-cover" />}
            <div class="flex-grow">
                ...
                <p class="text-sm my-2" style={{ 'color': 'var(--text-color)' }}>{props.project.description}</p>
                <div class="text-xs mt-3 flex flex-wrap gap-x-4 gap-y-2" style={{ 'color': 'var(--bright-black)' }}>
                    <span><span style={{'color': 'var(--text-color)'}}>⬇️</span> Downloads: <span class="font-semibold" style={{ 'color': 'var(--yellow)' }}>{props.project.downloads.toLocaleString()}</span></span>
                    <span><span style={{'color': 'var(--text-color)'}}>🏷️</span> Type: <span class="font-semibold capitalize" style={{ 'color': 'var(--blue)' }}>{props.project.project_type}</span></span>
                    <span><span style={{'color': 'var(--text-color)'}}>🗓️</span> Published: <span class="font-semibold" style={{ 'color': 'var(--cyan)' }}>{formatDateWithRelativeTime(props.project.published)}</span></span>
                    <span><span style={{'color': 'var(--text-color)'}}>🔄</span> Updated: <span class="font-semibold" style={{ 'color': 'var(--magenta)' }}>{formatDateWithRelativeTime(props.project.updated)}</span></span>
                </div>
            </div>
        </div>
    );
...
const PaginatedProjectsView: Component<PaginatedProjectsViewProps> = (props) => {
    ...
    return (
        <div>
            <UserProfile user={props.userProfile} projectCount={props.projectCount} totalDownloads={props.totalDownloads} />
...
export const handler = async (
    ...
    try {
        ...
        if (!userResponse.ok) throw new Error(`Could not fetch user profile. (Status: ${userResponse.status})`);
        if (!projectsResponse.ok) throw new Error(`Could not fetch projects. (Status: ${projectsResponse.status})`);

        const userProfile: ModrinthUser = await userResponse.json();
        const projects: ModrinthProjectData[] = (await projectsResponse.json()).sort((a: ModrinthProjectData, b: ModrinthProjectData) => b.downloads - a.downloads);
