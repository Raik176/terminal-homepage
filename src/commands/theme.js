    ...
    arguments: [
        {
            ...
            type: {
                typeName: "theme",
                validate: (value) => Object.keys(Themes).includes(value),
                parse: (val) => val,
                suggestions: (currentInput) => Object.keys(Themes).filter(theme => theme.startsWith(currentInput))
            }
        }
    ],
};
...
export const handler = (terminal, args) => {
    ...
    for (const themeName of themeNames) {
        ...
        const normalColors = colorNames.map(c => colorBlock(theme[`--${c}`])).join('');
        const brightColors = colorNames.map(c => colorBlock(theme[`--bright-${c}`])).join('');
