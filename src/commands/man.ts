...
export const handler = (
	...
	const generateArgumentsTable = (
		...
		argumentDefs.forEach((arg) => {
			const argumentText = arg.shorthand
				? `-${arg.shorthand}, --${arg.name}`
				: `--${arg.name}`;
			const typeName =
				typeof arg.type === "string"
					? arg.type
					: arg.type.typeName || "custom";
			columnWidths[0] = Math.max(columnWidths[0], argumentText.length);
			columnWidths[1] = Math.max(
				columnWidths[1],
				String(arg.optional ? "yes" : "no").length
			);
		...
		const tableContent = argumentDefs
			.map((arg) => {
				const argumentText = arg.shorthand
					? `-${arg.shorthand}, --${arg.name}`
					: `--${arg.name}`;
